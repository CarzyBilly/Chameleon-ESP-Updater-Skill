...
这是一个给 Codex 使用的自定义 skill，用来在 MECCHA CHAMELEON 更新后，按固定流程快速修复和重新适配 [phxgg/chameleonEsp](https://github.com/phxgg/chameleonEsp)。它有两条路线：如果原作者项目已经适配当前游戏版本，就在作者的完整源码上打补丁，保留原项目功能，同时修复已验证的问题并加入中英双语、中文字体等增强；如果游戏版本已经超过原作者当前版本，则从 0 开始完成找游戏、准备工具、Dump SDK、阅读 SDK、分阶段构建、测试、读取崩溃日志和继续修复。
...
这个 skill 是自包含流程，不需要也不会依赖我电脑上以前做过的增强包、旧 DLL、旧源码或测试目录。原作者同版本路线会重新下载/使用干净的 `phxgg/chameleonEsp` 源码，然后根据 skill 里的规则现场打补丁；游戏版本更高时才进入重新 dump SDK 的从 0 路线。

走“原作者同版本源码补丁”路线时，Codex 不应该去读取或复制任何历史最终包，例如旧增强源码、旧 DLL、旧 SDK dump、`D:\重要文件` 或之前的 `chameleon-work`。过去验证过的修复只能以 skill 里的规则形式复现到干净源码上。从 0 路线仍按重新找游戏、准备工具、dump SDK、读 SDK 和分阶段构建执行。
...
本仓库也不包含个人测试生成的增强源码、SDK dump 或 DLL。每次运行都应从当前原作者源码、当前游戏文件、当前 SDK 证据和本 skill 的补丁规则重新生成结果。
